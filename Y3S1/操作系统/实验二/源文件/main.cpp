...
int main() {
    ...
    sharedMemoryHandle = CreateFileMapping(
            ...
            PAGE_READWRITE,        // 读写访问
            0,                     // 最大对象大小（高32位）
            sizeof(struct BufferPool), // 最大对象大小（低32位）
    ...
    // 创建空信号量
    HANDLE emptySemaphoreHandle = CreateSemaphore(nullptr, 6, 6, emptySemaphoreName);
    ...
    // 创建满信号量
    HANDLE fullSemaphoreHandle = CreateSemaphore(nullptr, 0, 6, fullSemaphoreName);
    ...
    // 映射共享内存并清零
    struct BufferPool *sharedBuffer = (struct BufferPool *) MapViewOfFile(sharedMemoryHandle, FILE_MAP_ALL_ACCESS, 0, 0, MEMORY_BUFFER_LEN);
    if (sharedBuffer == nullptr) {
        printf("共享内存映射视图失败\n");
        CloseHandle(sharedMemoryHandle);
        return 1;
    }
    memset(sharedBuffer, 0, sizeof(struct BufferPool)); // 清零共享内存
    UnmapViewOfFile(sharedBuffer); // 解除映射
    sharedBuffer = nullptr;

    // 进程信息数组
    PROCESS_INFORMATION processInfoArray[PRODUCER_COUNT + CONSUMER_COUNT] = {0};
    STARTUPINFO startupInfoArray[PRODUCER_COUNT + CONSUMER_COUNT] = {0};
    ...
    HANDLE processHandles[PRODUCER_COUNT + CONSUMER_COUNT];
    DWORD exitCode;
    for (int i = 0; i < PRODUCER_COUNT + CONSUMER_COUNT; i++) {
        processHandles[i] = processInfoArray[i].hProcess; // 存储进程句柄
    }
    // 等待所有子进程结束
    WaitForMultipleObjects(PRODUCER_COUNT + CONSUMER_COUNT, processHandles, TRUE, INFINITE); // 等待所有进程结束
    printf("主进程退出!\n");
    for (int i = 0; i < PRODUCER_COUNT + CONSUMER_COUNT; i++) {
